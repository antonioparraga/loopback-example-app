...
Once we have created our entities we can call them from a really complete API provided by Loopback.
For example, to create new Patients:

```javascript

var patient = {SIP: '12345', 
               phone: '55555555', 
               name: 'John Rambo'}

Patient.create(patient, function(err, persistedPatient) {
    console.log(persistedPatient.id); //id is surrogated and autogenerated by the engine                
});
```

For example, to find patients:
```javascript

//search by SIP
Patient.findOne({where: { SIP: '12345' }}, function(err, patient) {
...
//search by name
Patient.find({where: { name: 'John Rambo' }}, function(err, patients) {
    console.log(patients.length);            
}
```

The API by default is really [complete](https://loopback.io/doc/en/lb2/Working-with-data.html) and covers most common CRUD operations. It can be extended and customized, however it's really useful to have this API from the beginning for rapid prototyping purposes, which is the DNA of Agile.

Because the application is isomorphic, the same API can be used from client side without calling explicitelly to the server rest API. Loopback in fact generates a library that handles this comunication depending on the declared sync policies and make it transparent to the developer.
